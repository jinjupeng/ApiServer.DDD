using ApiServer.Infrastructure.Exceptions;
using ApiServer.Infrastructure.Result;
...
namespace ApiServer.Infrastructure.Filters
{
    /// <summary>
    ...
    public class GlobalExceptionFilter : IExceptionFilter
    {
        ...
        {
            IResultModel msgModel;
            var exception = context.Exception;
            if (exception is CustomException)
            {
                var customException = exception as CustomException;
                msgModel = ResultModel.Failed(customException.Msg);
            }
            else
            {
                msgModel = ResultModel.Failed(exception.Message);
            }
            ...
            // 采用Serilog日志框架记录
            _logger.LogError(msgModel.Msg, WriteLog(msgModel.Msg, exception));
